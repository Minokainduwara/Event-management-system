...
function AvailabilityBar({ registered, max }: { registered: number; max: number }) {
  ...
  const label =
    pct >= 90 ? "Almost full" : pct >= 65 ? "Filling up" : "Open";
  const labelColor =
    pct >= 90 ? "text-red-600" : pct >= 65 ? "text-amber-600" : "text-emerald-600";
    ...
    <section className="max-w-7xl mx-auto px-6 py-12">
      ...
      {!isFiltered && events.length > 0 && (
        <div className="mt-10 text-center">
          <Link
            to="/student/events"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-violet-600 text-white text-sm font-medium hover:bg-violet-700 transition"
          >
            Browse all events
