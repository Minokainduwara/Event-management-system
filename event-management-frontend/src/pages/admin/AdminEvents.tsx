...
function AdminEvents() {
  ...
  const handleSearch = (value: string) => {
    ...
    if (value === "") {
      ...
      apiFetch(`http://localhost:8080/events/searchEvent?keyword=${value}`)
        .then((res) => res.json())
        .then((data) => setEvents(data))
        .catch((err) => console.log(String(err)));
    }
  ...
  const handleCategoryChange = (value: string) => {
    ...
    if (value === "") {
      ...
      apiFetch(`http://localhost:8080/events/filter?categoryId=${value}`)
        .then((res) => res.json())
        .then((data) => setEvents(data))
        .catch((err) => console.log(String(err)));
    }
  ...
  const handleDelete = (id: number) => {
    ...
    apiFetch(`http://localhost:8080/events/deleteEvent/${id}`, {
      method: "DELETE",
    })
      .then(() => {
