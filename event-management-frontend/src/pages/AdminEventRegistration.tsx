...
function AdminEventRegistration() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/eventRegistrations/summary")
      .then((res) => res.json())
  ...
  const searchRegistrations = (keyword: string) => {
    fetch(`http://localhost:8080/eventRegistrations/search?keyword=${keyword}`)
      .then((res) => res.json())
  ...
  const fetchAllRegistrations = () => {
    fetch("http://localhost:8080/eventRegistrations/registration")
      .then((res) => res.json())
      .then((data) => {
  ...
  const filterByStatus = (status: string) => {
    if (status === "") {
      ...
      fetch(`http://localhost:8080/eventRegistrations/filter?status=${status}`)
        .then((res) => res.json())
        .then((data) => setRegistrations(data))
        .catch((err) => console.log(err));
    }
  ...
  const handleRemove = (id: number) => {
    fetch(`http://localhost:8080/eventRegistrations/${id}`, {
      method: "DELETE",
    })
      .then(() =>
