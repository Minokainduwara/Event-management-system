...
function ViewStudent() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/users/students/count")
      .then((res) => res.json())
  ...
  const fetchAllStudents = () => {
    fetch("http://localhost:8080/users/getAllStudents")
      .then((res) => res.json())
      .then((data) => {
  ...
  const searchStudents = (keyword: string) => {
    fetch(`http://localhost:8080/users/students/search?keyword=${keyword}`)
      .then((res) => res.json())
  ...
  const filterByDepartment = (dept: string) => {
    if (dept === "") {
      ...
      fetch(`http://localhost:8080/users/students/department/${dept}`)
        .then((res) => res.json())
        .then((data) => setStudents(data))
        .catch((err) => console.log(err));
    }
  ...
  const handleDelete = (id: number) => {
    fetch(`http://localhost:8080/users/deleteUser/${id}`, { method: "DELETE" })
      .then(() => {
